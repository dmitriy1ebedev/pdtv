...
_mv_into() {
    ...
    local err=""
    err="$(mv --backup=numbered -t "$dir" -- "$@" 2>&1)" || true
    # ПОЧЕМУ ПРОВЕРЯЕМ ФАКТ, А НЕ КОД ВОЗВРАТА. mv может не справиться молча или
    # частично: нет прав на общий каталог, кончилось место, файл держит Dr.Web,
    # PARSEC-метка, слишком длинное имя. Раньше это глушилось «|| true», а M6
    # следом удалял «Загрузку» — документ исчезал, а в логе значилось «без
    # предупреждений». Теперь о каждом неперенесённом файле говорим вслух;
    # само удаление подстраховано проверкой в final_cleanup.
    #
    # Код возврата ОСТАВЛЯЕМ нулевым намеренно: функция вызывается в связках
    # «(( … )) && _mv_into …», где ненулевой код под set -e уронил бы весь прогон.
    local f stuck=0
    for f in "$@"; do
        [[ -n "$f" && -e "$f" ]] || continue
        warn "  НЕ перенесён в ${dir##*/}/: ${f##*/}"
        stuck=$(( stuck + 1 ))
    done
    # Причину печатаем ОДИН раз: в $err лежит общий вывод mv по всей пачке, и
    # приписывать его каждому файлу — врать (у второго файла может быть другая
    # причина или её вовсе нет).
    (( stuck )) && [[ -n "$err" ]] && warn "    причина: ${err//$'\n'/; }"
    return 0
}
...
# Как run, но неуспех ФАТАЛЕН. Для действий, без которых прогон бессмыслен:
# создание рабочих каталогов. Раньше это шло через терпимый run — если mkdir
# не проходил (нет прав на общий каталог, PARSEC-метка, кончилось место),
# скрипт молча работал дальше с несуществующими путями: архивы не
# распаковывались, ПДТВ выходил пустым, а оператор видел «Обработка завершена».
run_must() {
    if [[ "$DRY_RUN" == true ]]; then log "[would] $*"; return 0; fi
    "$@" || die "не удалось выполнить (код $?): $*"
}
...
prepare_environment() {
    ...
    # Без этих каталогов работать нечем — падаем сразу и понятной ошибкой,
    # а не «успешным» прогоном с пустым ПДТВ (см. run_must).
    run_must mkdir -p "$PDTV_P" "$ZAGRUZKA" "$DEEP_DIR"
...
_decompress() {
    ...
    # Распаковываем ЧЕРЕЗ СТДАУТ в свободное имя, а не штатным «-dk».
    #
    # ПОЧЕМУ. У «$tool -dk file.gz» цель жёстко равна имени без расширения, и если
    # такой файл уже есть, возможны только два плохих исхода: без -f архиватор
    # спрашивает «overwrite?» (в терминале прогон висит вечно, под ярлыком stdin
    # закрыт → код ≠0 и архив числится нераспакованным), а с -f он МОЛЧА затирает
    # чужой документ с тем же именем. Второе нашёл фаззер: рядом с
    # «отчёт о работе.txt.gz» лежал другой «отчёт о работе.txt», и он исчезал.
    #
    # Поэтому целевое имя подбираем сами: занято — добавляем (2), (3), …
    local dst="${f%.*}" n=2
    while [[ -e "$dst" ]]; do
        local base="${f%.*}" ext=""
    ...
    if ! "$tool" -dc -- "$f" > "$dst" 2>/dev/null </dev/null; then
        rm -f -- "$dst" 2>/dev/null || true   # не оставляем обрезок от сбоя
        return 2
    fi
    _mv_into "$OTRABOTKA_P" "$f"
    return 0
...
final_cleanup() {
    ...
    # Удаление каталога «Загрузка» — ТОЛЬКО когда в нём не осталось файлов.
    # Раньше удаляли безусловно: если перенос выше не удался (см. _mv_into),
    # документы уходили в небытие, а оператор видел «Обработка завершена».
    # На офлайн-АРМ восстановить их неоткуда, поэтому лучше оставить каталог
    # и сказать об этом, чем удалить молча.
    if [[ "$DRY_RUN" == true ]]; then
        log "[would] rm -r: $ZAGRUZKA"
    else
        local -a stuck=()
        mapfile -d '' stuck < <(find "$ZAGRUZKA" -type f -print0 2>/dev/null || true)
        if (( ${#stuck[@]} )); then
            warn "  «Загрузка» НЕ удалена — в ней остались файлы (${#stuck[@]}). Разберите вручную:"
            warn "    $ZAGRUZKA"
            local f
            for f in "${stuck[@]}"; do
                [[ -n "$f" ]] && warn "      • ${f#"$ZAGRUZKA"/}"
...
_diag_advice() {
    ...
    # НЕ обязателен: обработка идёт с сохранёнными настройками. На ОФЛАЙН-АРМ
    # ставить его нельзя — apt без репозитория рушит рабочий стол (лог юзера 20.07).
    if ! command -v zenity >/dev/null 2>&1; then
        echo "• Нет zenity — окно настроек не показывается, ПДТВ отрабатывает с сохранёнными настройками (это НЕ ошибка)."
        echo "  → на офлайн-машине НЕ ставьте его (apt может снести рабочий стол); настройки задавайте ключами: pdtv.sh --root … --fio …"
    fi
    ...
    if [[ -n "$dsk" ]]; then
        ...
        if [[ -z "$dpath" ]] || ! command -v "$dbin" >/dev/null 2>&1; then
            bad=1
            echo "⚠ Ассоциация text/plain ведёт на «$dsk», а программы нет — потому ПДТВ и «не открывается» (на 1.7 ассоциация была цела)."
            echo "  → офлайн-фикс БЕЗ интернета: назначить УЖЕ УСТАНОВЛЕННЫЙ редактор:"
            echo "    ls /usr/share/applications | grep -iE 'edit|kate|kwrite|mousepad|gedit|text'   — выбрать существующий .desktop"
            echo "    xdg-mime default ВЫБРАННЫЙ.desktop text/plain"
        fi
    fi
...
# ярлыку молча шёл в обработку без окна настроек («не даёт выбрать»). Объясняем
# оператору, что это НЕ ошибка (настройки сохранены), и что на офлайн-АРМ ставить
# zenity нельзя — apt без репозитория рушит рабочий стол (лог юзера 20.07).
...
# Экранирование значения для строки Exec= в .desktop (Desktop Entry Specification).
# ЗАЧЕМ. Путь и ФИО приходят из формы zenity, то есть это ввод пользователя,
# который попадает в исполняемую строку ярлыка:
#   • перевод строки позволил бы дописать произвольные ключи [Desktop Entry]
#     (в том числе ВТОРОЙ Exec=) — вырезаем управляющие символы;
#   • внутри кавычек спец-символы " ` $ \ по спецификации экранируются обратной
#     косой, иначе строка Exec рвётся и ярлык молча перестаёт работать;
#   • % — это field code (%U, %f, %i…), система подставит вместо него своё,
#     поэтому удваиваем.
_desktop_esc() {
    local s="$1"
    s="${s//[$'\n\r\t\v\f']/ }"   # управляющие → пробел (порядок важен: до кавычек)
    s="${s//\\/\\\\}"             # обратная косая — первой, иначе экранируем свои же
    s="${s//\"/\\\"}"
    s="${s//\`/\\\`}"
    s="${s//\$/\\\$}"
    s="${s//%/%%}"
...
build_cli_flags() {
    local f=""
    f+=" --root \"$(_desktop_esc "$ROOT")\""
    [[ -n "$OFFICER_CLI" ]] && f+=" --officer \"$(_desktop_esc "$OFFICER_CLI")\""
...
make_home_shortcut() {
    ...
    # Name попадает в файл как есть — вырезаем переводы строк, иначе ФИО с
    # переносом дописало бы в [Desktop Entry] произвольные ключи.
    name="${name//[$'\n\r']/ }"
...
Exec=bash "$(_desktop_esc "$script")"$flags
Terminal=false
...
#
# Версия команды для отчёта. У диалоговых программ не вызываем `--version`:
# некоторые из них (на Astra 1.8 это xmessage) воспринимают его как обычный
# текст и ОТКРЫВАЮТ ПУСТОЕ ОКНО. Диагностика должна быть полностью
# безвзаимодействующей: отчёт собирают удалённо, а не сидят у АРМ, чтобы
# закрывать каждое неожиданное окно.
_diagnostic_tool_version() {
    local c="$1"
    case "$c" in
        zenity|kdialog|fly-dialog|Xdialog|dialog|whiptail|xmessage|notify-send)
            printf '%s' '<диалоговая утилита; версию не запускаем>'
            return 0
            ;;
    esac
...
run_diagnostic() {
    ...
    (
        ...
        for c in bash find sed sort paste grep tr wc file stat \
            ...
            if [[ -n "$p" ]]; then
                ver="$(_diagnostic_tool_version "$c")"
                printf '%-11s %s | %s\n' "$c" "$p" "${ver:-<нет --version>}"
            else
        ...
        echo "--- 13. Окружение (переменные, влияющие на работу) ---"
        # ТОЛЬКО белый список. Полный env сюда выгружать нельзя: отчёт создаётся,
        # чтобы передать его наружу «тому, кто разбирает проблему», а в окружении
        # живут токены прокси, *_PASSWORD, *_TOKEN, пути к ключам. Если понадобится
        # переменная, которой тут нет, — дописать в шаблон ниже, а не звать env.
        env 2>/dev/null | LC_ALL=C sort | grep -E \
            '^(LANG|LANGUAGE|LC_[A-Z_]+|TZ|DISPLAY|WAYLAND_DISPLAY|XDG_[A-Z_]+|DESKTOP_SESSION|GDMSESSION|TERM|COLORTERM|NO_COLOR|SHELL|HOME|USER|LOGNAME|PWD|PATH|EDITOR|GNUPGHOME|PDTV_[A-Z_]+)=' \
            || echo "(ни одна из ожидаемых переменных не задана)"
        echo
